...
class PlayerORM(Base):
    ...
    entries: Mapped[List["EntryORM"]] = relationship(
        "EntryORM", foreign_keys="EntryORM.player_email"
    )

    created_events: Mapped[List["EventORM"]] = relationship(
...
class EntryORM(Base):
    ...
    ss_recipient_email = Column(
        Integer, ForeignKey("players.email"), nullable=True, default=None, index=False
    )
